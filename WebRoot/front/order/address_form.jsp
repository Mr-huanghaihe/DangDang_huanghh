...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javaScript">
		    ...
		    window.location.href="${pageContext.request.contextPath}/user/oneAddress?id="+id;  
		};
		</script>
	</head>
	<body>
		...
		<div class="fill_message">
			<p>
				选择地址：
				<select id="address" name="address" onchange="selectAddress(this.value)">
					<option value="">填写新地址</option>
				<c:forEach var="l" items="${list }">
					<option value="${l.id }">${l.local }</option>
				</c:forEach>
				</select>
			</p>
			<form name="ctl00" method="post" action="${pageContext.request.contextPath}/user/addOrderItem" id="f">
